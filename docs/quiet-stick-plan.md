...
which RetroArch 1.22 never prints (and without verbose logging it prints nothing at all) - the splash
always waited its full 30 s. Fixed on 2026-09-24: `retroarch_run` passes `--verbose` and the splash waits
for "[Video] Found display server", printed right after the video driver's init (the 30 s stay as the
fallback).
