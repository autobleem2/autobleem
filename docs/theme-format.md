...
Themes/<name>/
  ...
  images/launcher_background.png  launcher_footer.png  play_button.png  play_text.png
         ...
         memcard_grid.png  memcard_pencil.png                 the EvolutionUI launcher (names are free
                                                              ...
                                                              the shipped themes' are drawn by
                                                              tools/make_theme_images.py)
  font/medium.ttf  font/bold.ttf                              optional - the launcher's own pair
  sounds/cursor.wav  cancel.wav  home_up.wav  home_down.wav  resume_new.wav      optional - the default theme's
                                                              are generated by tools/make_theme_sounds.py
...
| `classic.editorCover` | Not read since 2026-09-21: the game editors and the Game Manager draw the cover in a pane at the right of the panel (`GameDetailPane`). Kept in the format for older themes. |
...
| `launcher.hintBar` | `{ "x", "y", "w", "h" }`: the frame the footer's hint row (Enter, Cancel, Button Guide, L2+R2 Options, Start Random) is laid out in - centred, at the largest font size from 22 down to 14 that fits the language, so a long translation shrinks instead of spilling. Unset means the pill most themes paint at the bottom right: x 560..1240, y 624..696. ab2 gives its whole blue band, the default theme its strip. |
